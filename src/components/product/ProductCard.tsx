...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <Card className="group overflow-hidden transition-all duration-200 hover:shadow-md aspect-[4/3] bg-background animate-fade-in border border-border rounded-none hover:border-primary/20">
      ...
      <div className="h-1/6 flex items-center justify-start px-3 py-2 bg-background">
        <Link to={`/product/${product.id}`} className="w-full">
          <h3 className="text-foreground font-normal text-sm hover:text-primary transition-colors duration-150 truncate text-left leading-tight">
            {product.title}
          </h3>
        </Link>
      </div>
