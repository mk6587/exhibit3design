...
Format the response as JSON:
{
  ...
  "readabilityScore": 75
    ...
    try {
      ...
      articleData.content = articleData.content.replace(/```json[\s\S]*?```/g, '').replace(/^\s*\{[\s\S]*?"content":\s*"/, '').replace(/",\s*"keywords"[\s\S]*$/, '').trim();
      ...
      const cleanContent = contentText.replace(/```json[\s\S]*?```/g, '').replace(/^\s*\{[\s\S]*?"content":\s*"/, '').replace(/",\s*"keywords"[\s\S]*$/, '').trim();
