...
-- Fix the policy creation with proper name handling
...
-- Create a secure policy with a unique name that allows service role to update orders
CREATE POLICY "Restricted service role order updates" 
ON public.orders 
FOR UPDATE 
USING (true);
...
DROP TRIGGER IF EXISTS validate_order_updates ON public.orders;
